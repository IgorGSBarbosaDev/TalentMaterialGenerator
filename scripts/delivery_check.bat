...
python -c "import sys; assert hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix), 'Not in venv'" 2>nul
if %ERRORLEVEL% NEQ 0 (
...
black --check --diff app/ tests/ --quiet
if %ERRORLEVEL% NEQ 0 (
...
flake8 app/ tests/ --max-line-length=%MAX_LINE_LENGTH% --extend-ignore=E203,W503 --statistics
if %ERRORLEVEL% NEQ 0 (
...
mypy app\ --ignore-missing-imports --python-version=3.11 --no-error-summary
if %ERRORLEVEL% NEQ 0 (
...
coverage report --include="app/core/*" --fail-under=%COVERAGE_CORE_MIN% -m
if %ERRORLEVEL% NEQ 0 (
